...
public class PdfFolderService {
    ...
    public List<ExportDeclaration> processFolder(File folder) throws Exception {
        // 1) run the extractor
        ...
        List<String> expected = List.of("numeExportator", "mrn", "dataDeclaratie", "nrContainer");

        List<Map.Entry<String, ExportDeclaration>> rows = new ArrayList<>();
        ...
        for (JsonNode n : arr) {
            ...
            ExportDeclaration dto = mapper.treeToValue(n, ExportDeclaration.class);
            rows.add(new AbstractMap.SimpleEntry<>(fileName, dto));
        }

        rows.sort((a, b) -> naturalCompareIgnoreCase(a.getKey(), b.getKey()));

        List<ExportDeclaration> good = new ArrayList<>(rows.size());
        for (var e : rows) {
            logger.accept("✅ Parsed successfully: " + e.getKey());
            logger.accept("Value: " + e.getValue());
            good.add(e.getValue());
        }
